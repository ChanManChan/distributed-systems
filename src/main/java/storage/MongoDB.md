...
![mongo chunks example](assets/mongo_chunks_example.jpg) <br />

Since we set the maximum size for chunk as 1MB (db.settings.save({ _id: "chunksize", value: 1 }) ), and we definitely
exceeded that threshold, now we have 14 chunks of documents in our collection (2 in image). We can also see that the
balancer placed 7 of them in shard-0 and the other 7 in shard-1. In addition, we can see the exact range of names in
each chunk and altogether the names are stored in alphabetical order just like we expected. Now if we were to query all
the movies that starts with the letter 'Z', we will find all of them in the same shard despite the fact that they span 2
separate chunks. This is because we chose the range based sharding strategy for our names.
...
* We will use hash based sharding strategy
    * No need for groups of users to be co-located on the same shard
    * We want even and random distribution of users across the shards so that reads and writes can scale all across the
      cluster
* User-names are not uniformly distributed since some names or letters are more popular than others
* Document IDs (generated by MongoDB) are random and uniformly distributed and are excellent candidate for a sharding
  key
* We need to refer to the user by ID to make the queries efficient
...
db.users.insertOne({
  ...
  "subscription_month": 7
})

db.users.find().pretty()
# each object id contains a 12 Byte value shown to us in a hexadecimal format
# shard the users collection on that id but this time using the hash sharding strategy
# Create an index for the id like before but this time we are going to create an index that pre hashes the values of our userIds
db.users.createIndex({ _id: "hashed" })
# once we have index, we shard the users collection by id using the hashed strategy 
sh.shardCollection("videodb.users", { _id: "hashed" })
sh.status(true)
```

![users chunk distribution](assets/users_chunk_distribution.jpg) <br />

Users chunk even distribution among the shards. Our balancer is still working. What we see here is not the ids
themselves but their hashes which are simply 64-bit integers. Our documents are now evenly distributed among all the
shards based on the hash function that was applied on the userId.

**Video Streaming - System Evolution**

![initial architecture](assets/initial_architecture.jpg) <br />

1. **More Reads from All Movies** <br />
   ![reads from all movies](assets/reads_from_all_movies.jpg) <br />
   As our service becomes more popular, and we start getting more read queries for all the movies, that means our load
   increases evenly on all the shards. One approach we can take is simply increase the number of shards. This way the
   load on each shard will decrease and keep our distributed system stable.
2. **More Reads from some Movies** <br />
   ![reads from some movies](assets/reads_from_some_movies.jpg) <br />
   Our load increases but one or few of our movies are way more popular than the rest and are getting disproportionately
   large number of queries, in this case adding more shards will not solve the problem as we have a clear hotspot in our
   dataset. Instead, what we can do is increase the size of the replication set and in addition use the read
   preference "secondaryPreferred", this way we will spread the queries among all the nodes in the shards' replication
   set as they all contain the same data for that particular movie. This approach will work well because our movies data
   doesn't change very often, and we don't need to guarantee strict consistency in this use case.
3. **More Operations on Users** <br />
   ![operations on users](assets/operations_on_users.jpg) <br />
   When it comes to scaling our users collection it is actually a lot simpler. If the number of users keeps increasing,
   and we are getting more logins from existing users and signups from new users, we can simply increase the number of
   shards. Since our userIds are uniformly distributed we are guaranteed to not have any hotspots in our distributed
   database.
